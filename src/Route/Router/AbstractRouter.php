 ...
 * @package Grace\Swoft\Route\Router
...
abstract class AbstractRouter implements RouterInterface
{
    ...
    public function __call($method, $args)
    {
        if (\in_array(\strtoupper($method), self::ALLOWED_METHODS(), true)) {
            ...
            $params = [$method] + $args;
            return  call_user_func_array(array($this, "map"), $params);
     ...
     * @param array $opts
     */
    public function group($prefix, $callback, $opts = [])
    {
     ...
     * @param array $conf
     */
    protected function filterMatches($matches, &$conf)
    {
        if (!$matches) {
            $conf['matches'] = [];
            return;
     ...
     * @throws \LogicException
     */
    public function parseParamRoute($route, $params, $conf)
    {
        $bak = $route;
        $noOptional = null;
        ...
        if (\preg_match_all('#\{([a-zA-Z_][a-zA-Z0-9_-]*)\}#', $route, $m)) {
            ...
            foreach ($m[1] as $name) {
                $key = '{' . $name . '}';
                $regex = !empty($params[$name]) ? $params[$name] : self::DEFAULT_REGEX;
     ...
     * @return array
     */
    abstract protected function findInRegularRoutes($routesData, $path, $method);
     ...
     * @return array
     */
    abstract protected function findInVagueRoutes($routesData, $path, $method);

    /**
     * @param array $tmpParams
     * @return array
     */
    public function getAvailableParams($tmpParams)
    {
     ...
     */
    public function setRegularRoutes($regularRoutes)
    {
