...
use Grace\Swoft\Route\Router\HandlerMappingInterface;
...
class HandlerMapping extends AbstractRouter implements HandlerMappingInterface
{
     ...
     */
    public function map($methods, $route, $handler, $opts = [])
    {
     ...
     * @return array
     */
    protected function prepareForMap($route, $handler, $opts)
    {
        if (!$this->initialized) {
     ...
     * @throws \LogicException
     */
    protected function collectParamRoute($route, $methods, $conf)
    {
        $conf['original'] = $route;
        $params = !empty($this->getAvailableParams($conf['option']['params']) ?  : []);
     ...
     * @return array
     */
    protected function findAllowedMethods($path, $method, $allowedMethods)
    {
     ...
     * @return array
     */
    protected function findInRegularRoutes($routesData, $path, $method)
    {
        $allowedMethods = '';
     ...
     * @return array
     */
    protected function findInVagueRoutes($routesData, $path, $method)
    {
        foreach ($routesData as $conf) {
     ...
     * @throws \InvalidArgumentException
     */
    public function registerRoutes($requestMapping)
    {
     ...
     * @throws \InvalidArgumentException
     */
    private function registerRoute($className, $routes, $controllerPrefix)
    {
     ...
     * @return string
     */
    private function getControllerPrefix($controllerPrefix, $className)
    {
        // 注解注入不为空，直接返回prefix
