...
namespace Grace\Swoft\Proxy;

use Grace\Swoft\Proxy\Handler\HandlerInterface;
...
class Proxy
{
     ...
     */
    public static function newProxyInstance($className, $handler)
    {
     ...
     * @return string
     */
    private static function getMethodsTemplate($reflectionMethods, $handlerPropertyName)
    {
        $template = '';
        foreach ($reflectionMethods as $reflectionMethod) {
            ...
            $reflectionMethodReturn = method_exists ($reflectionMethod , "getReturnType") ? $reflectionMethod->getReturnType() : null;
     ...
     * @return string
     */
    private static function getParameterTemplate($reflectionMethod)
    {
        $template = '';
        ...
        foreach ($reflectionParameters as $reflectionParameter) {
            ...
            if ($reflectionParameter->isPassedByReference()) {
                $template .= " &\${$paramName} ";
            } elseif (method_exists($reflectionParameter, "isVariadic") && $reflectionParameter->isVariadic()) {
                $template .= " ...\${$paramName} ";
            } else {
                $template .= " \${$paramName} ";
            }
            ...
            if (($reflectionParameter->isOptional() && !method_exists($reflectionParameter, "isVariadic")) || ($reflectionParameter->isOptional() && $reflectionParameter->isVariadic() === false)) {
     ...
     * @return string
     */
    private static function getParameterDefaultValue($reflectionParameter)
    {
        $template = '';
