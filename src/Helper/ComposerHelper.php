...
class ComposerHelper
{
     ...
     */
    public static function findLoader()
    {
        $composerClass = '';
     ...
     * @return string
     */
    public static function getDirByNamespace($namespace)
    {
        $dir = '';
        $loader = self::findLoader();
        ...
        foreach ($prefixesPsr4 as $prefix => $path) {
            if (StringHelper::startsWith($namespace, $prefix)) {
                ...
                if ($strLen > $maxLength) {
                    $dir = current($path) . DIRECTORY_SEPARATOR . substr($namespace, strlen($prefix));
